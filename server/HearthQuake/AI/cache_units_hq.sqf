...
if (ai_cache_units_freeze_log_hq) then {
...

{
  if(alive _x) then {
   ...
   _x disableAI "ANIM";
   sleep 0.05;
   if (ai_cache_units_unassign_waypoints_hq) then {
    _x disableAI "FSM";
    sleep 0.05;
   };

   if (ai_cache_unites_hide_ai_hq) then {
...

{
  ...
  if (ai_cache_units_randomize_position_hq) then {
   _pos = getPos _x;
   _posX = _pos select 0;
   _posY = _pos select 1;
   _posZ = _pos select 2;
   _posX = _posX + round(random ai_cache_units_randomize_distance_hq);
   _posY = _posY + round(random ai_cache_units_randomize_distance_hq);
   sleep 0.05;
...
**/
while {true} do {
_matchingObjectsArray = ((units _unitGroup) select 0) nearEntities ["CAManBase",ai_cache_units_reactivation_range_hq];
if(!isnil "_matchingObjectsArray") then {
  ...
  if (_numberOfMatchingObjectsNumber >= 1) then {
   ...
   if (_stateFroze) then {
    ...
    if (!_playerPresent && ((time - _timeFroze) > ai_cache_units_refreeze_hq)) then {
     if (ai_cache_units_freeze_log_hq) then {
